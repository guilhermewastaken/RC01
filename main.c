    ...
    while(alarmEnabled == 1 && state != OVER){
        switch (state) {
            ...
            case BUILDING_HEADER:
                ...
                if (counter == 3) {
                    header = getHeaderType(header, parityReceived);
                    if (header == INVALID) {
                        state = WAIT_FOR_FLAG;
                        counter = 0;
                    }
            ...
            case INFO_FILLED:
                *size = removeStuffing(data, *size);
                if (getBCC(data, (*size) - 2) != data[(*size) - 2]) {
                    counter = 0;
                    sendPacket(REJ, 0, 0); //apagar se nao funcionar
                    state = WAIT_FOR_FLAG;
                }
                break;
        }
...
int main(int argc, char *argv[]) {
    machine = RECEIVER; //Or transmitter
    ...
    //send non canonical
    #define BUF_SIZE 256
    unsigned char arraySent[5] = {0x7e,  0x03, 0x03, 0x7e, 0x7e};
    arraySent[2] = 0xb;
    arraySent[3] = getBCC(arraySent,3);
    printByte(getBCC(arraySent,3));
    ...
    unsigned int size;
    unsigned int parity;
    receivePacket(data, size, parity);
